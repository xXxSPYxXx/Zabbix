...
if [ "${METRIC}" = "slowqueries" ]; then
  tail -n1000 /var/log/httpd/access_log|grep CPU_TIME|awk '{print $24,$25}'|perl -ne 'print if grep {$_>500000} /(\d{1,})/g'|grep -c .
fi
if [ "${METRIC}" = "slowqueriesmax" ]; then
  tail -n1000 /var/log/httpd/access_log|grep CPU_TIME|awk '{print $24,$25}'|perl -ne 'print if grep {$_>1000000} /(\d{1,})/g'|grep -c .
fi
#
exit 0
